...
const priceTransition = ({ $element, newPrice, duration }) => {
  ...
  const currentPriceInt = getMoney($element.textContent)
    ? getMoney($element.textContent) / 100
    : 0;
  // const newDuration = duration / 50;
  const newDuration = 50 / 50;
  ...
  const recursivePrint = (value, index) => {
    const formattedText = value.toLocaleString("pt-BR", {
      style: "currency",
      currency: "BRL",
    });
    ...
    fakeArray[index] &&
      setTimeout(() => recursivePrint(value + sumCoefficient, index + 1), 50);
  };
